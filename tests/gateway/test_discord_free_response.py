...
@pytest.mark.asyncio
async def test_discord_defaults_to_free_response_in_server_channels(adapter, monkeypatch):
...
@pytest.mark.asyncio
async def test_discord_defaults_to_free_response_in_threads(adapter, monkeypatch):
    monkeypatch.delenv("DISCORD_REQUIRE_MENTION", raising=False)
    monkeypatch.delenv("DISCORD_FREE_RESPONSE_CHANNELS", raising=False)
